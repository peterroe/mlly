...
import { isNodeBuiltin, sanitizeFilePath, getProtocol, parseNodeModulePath, resolveSubpath } from '../src'
...
describe('parseNodeModulePath', () => {
  it('parses paths', () => {
    const paths = ['/src/a/node_modules/thing/dist/index.mjs', 'C:\\src\\a\\node_modules\\thing\\dist\\index.mjs']
...
describe('resolveSubpath', () => {
  it('resolves correctly', async () => {
    const path = fileURLToPath(new URL('./fixture/node_modules/subpaths/lib/subpath.mjs', import.meta.url))
    const result = await resolveSubpath(path)
    expect(result).toMatchInlineSnapshot('"subpaths/subpath"')
  })
  it('ignores invalid paths', async () => {
    const path = fileURLToPath(new URL('./fixture/subpaths/lib/subpath.mjs', import.meta.url))
    const result = await resolveSubpath(path)
